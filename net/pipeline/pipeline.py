...
from net.model.safety_filter import SafetyAwareActionFilter
...
class Pipeline(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        # Safety-aware action filter: clamps predicted EE targets to satisfy
        # workspace, velocity, base-speed, smoothness and collision constraints
        # before they are consumed by the predictor. Disabled if config is
        # missing (keeps backwards compatibility with old config files).
        if hasattr(config, 'safety') and getattr(config.safety, 'enabled', False):
            self.safety_filter = SafetyAwareActionFilter(config.safety).cuda()
        else:
    ...
    def predict(self, z, data, loss_dict=None, acc_dict=None, postfix='_nxt', decode=True):
        ...
        # Apply the safety-aware action filter (if enabled) to the predicted EE
        # target before it is fed to the predictor. The dataset stores the EE
        # action as a point cloud, so we use its centroid as a pose surrogate
        # and translate the entire point cloud by the safety correction.
        ee_obs_full = data[f'ee_observed{ee_postfix}']
        ee_tgt_full = data[f'ee_target{ee_postfix}']
        if self.safety_filter is not None:
            ee_tgt_full, info = self._apply_safety_filter(ee_obs_full, ee_tgt_full,
                                                          obj_pts=data[f'obj_observed{ee_postfix}'],
                                                          prev_target=data.get(f'ee_target_safe_prev{ee_postfix}'))
            data[f'safety_info{ee_postfix}'] = info
            data[f'ee_target{ee_postfix}'] = ee_tgt_full
            # remember the filtered target so the next call's smoothness check
            # can compare against the previously *commanded* (safe) action
            data[f'ee_target_safe_prev{ee_postfix}'] = ee_tgt_full.detach()

        if self.config.model.condition.ee_shape == 'both':
            ee_observed = ee_obs_full[..., :self.config.dimensions.input_dim]
            ee_target = ee_tgt_full[..., :self.config.dimensions.input_dim]
    ...
    def _apply_safety_filter(self, ee_obs, ee_tgt, obj_pts, prev_target=None):
        """
        Translate the EE point cloud target so that its centroid satisfies the
        safety constraints encoded in self.safety_filter.

        ee_obs / ee_tgt: [B, N, 4]   point clouds (xyz + label)
        obj_pts:        [B, N_o, 4]  dough point cloud (xyz + label)

        Returns the filtered ee_tgt (same shape as input) and the per-batch
        violation info dict from the filter.
        """
        # centroid as position surrogate; identity quaternion (we have no
        # orientation labels in the dataset action representation)
        pos_obs = ee_obs[..., :3].mean(dim=1)  # [B, 3]
        pos_tgt = ee_tgt[..., :3].mean(dim=1)  # [B, 3]
        ones = torch.zeros_like(pos_obs[..., :1])
        quat_id = torch.cat([torch.ones_like(ones), ones, ones, ones], dim=-1)  # wxyz = (1,0,0,0)

        observed_pose = torch.cat([pos_obs, quat_id], dim=-1)
        target_pose = torch.cat([pos_tgt, quat_id], dim=-1)

        prev_pose = None
        if prev_target is not None and prev_target.shape == ee_tgt.shape:
            prev_pos = prev_target[..., :3].mean(dim=1)
            prev_pose = torch.cat([prev_pos, quat_id], dim=-1)

        safe_pose, info = self.safety_filter(
            observed=observed_pose,
            target=target_pose,
            ee_pts=ee_tgt[..., :3],
            obj_pts=obj_pts[..., :3],
            prev_target=prev_pose,
        )
        # apply correction to the entire EE point cloud (rigid translation)
        correction = (safe_pose[..., :3] - pos_tgt).unsqueeze(1)  # [B, 1, 3]
        ee_tgt_safe = ee_tgt.clone()
        ee_tgt_safe[..., :3] = ee_tgt_safe[..., :3] + correction
        return ee_tgt_safe, info
